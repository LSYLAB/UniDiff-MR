...
def main(args) -> None:
    ...
    grad_sd = torch.load('/data1/baihy/denoise/InverseSR/experiment-gradvae/checkpoints/0038000_gradvae.pt')
    ...
    # c_txt = torch.tensor([[gender, age_normalized, ventricular, brain_volume]],device=device)
    # c_txt.requires_grad = True
    # update_params.append(c_txt)
    ...
    # opt_cond = torch.optim.AdamW(cond_pretrain.parameters(), lr=0.0001)
    ...
    # epoch 训的轮次 一个epoch训多少个batch dataset.length/bachsize
    # 一个batch叫一个step
    # total step = epoch * dataset.length/bachsize
    global_step = 0
