...
def main(args) -> None:
    ...
    sd = torch.load('/data1/baihy/denoise/InverseSR/diffusion.pt', map_location="cpu")
    ...
    device = accelerator.device
    decoder = torch.load('/data1/baihy/denoise/InverseSR/weight/decoder/data/model.pth')
    ...
    grad_sd = torch.load('/data1/baihy/denoise/InverseSR/experiment-energyvae/checkpoints/0019000_gradvae.pt')
    ...
    if accelerator.is_local_main_process:
        ...
        for lq, lq_grad, info in val_loader:
            ...
            with torch.no_grad():  # 训练的时候 这下面的三个 都是相当于测试 全部冻住了（蓝色），只需要向前forward
                ...
                # if accelerator.is_local_main_process:
                #     print(z_gt.shape)
                cond, latent_variable = setup_noise_inputs(device=device, img = z_lq, batchsize = 1, c_txt = c_txt) 
            ...
            # 由于灰度图像只有一个通道，这里将最后一个维度去除了
                for i in range(lq.size(0)):
                    ...
                    zoom_factors = (512 / single_clean.shape[0], 512 / single_clean.shape[1], 164 / single_clean.shape[2])

                    # Resize the image data
                    resized_data = zoom(single_clean, zoom_factors)

                    print(single_clean.shape)
                    new_filename2 = f"{info['name'][i].split('.')[0]}"
                
                    # os.makedirs('/data1/baihy/denoise/InverseSR/results/ADNI/stage2_new/v2_4000/40', exist_ok=True)  # 确保目录存在
                    # original_volume = nib.load('/data1/baihy/denoise/data/ANDI/Gt_fsl_norm/test/0030_0.nii.gz')
                    # image_clean_nifti = nib.Nifti1Image(single_clean, affine=original_volume.affine)
                    # nib.save(image_clean_nifti, os.path.join('/data1/baihy/denoise/InverseSR/results/ADNI/stage2_new/v2_4000/40', new_filename2))

                    os.makedirs('/data1/baihy/denoise/InverseSR/results/IXI/stage2/v2_8500', exist_ok=True)  # 确保目录存在
                    original_volume = nib.load('/data1/baihy/denoise/data/ANDI/Gt_fsl_norm/test/0030_0.nii.gz')
                    image_clean_nifti = nib.Nifti1Image(single_clean, affine=original_volume.affine)
                    nib.save(image_clean_nifti, os.path.join('/data1/baihy/denoise/InverseSR/results/IXI/stage2/v2_8500', new_filename2))


                    # os.makedirs('/data1/baihy/denoise/InverseSR/results/ADNI/stage2_new/MRART_7000/', exist_ok=True)  # 确保目录存在
                    # original_volume = nib.load('/data1/baihy/denoise/data/ANDI/Gt_fsl_norm/test/0030_0.nii.gz')
                    # image_clean_nifti = nib.Nifti1Image(single_clean, affine=original_volume.affine)
                    # nib.save(image_clean_nifti, os.path.join('/data1/baihy/denoise/InverseSR/results/ADNI/stage2_new/MRART_7000/', new_filename2))
